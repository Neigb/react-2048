      ...
      <div className='row'>
        {
          col_array.map((value, col_index) => 
            <div key={col_index} className={"item flex-center color-" + value}>{ value ? <span>{ value }</span> : '' }</div>
          )
        }
      </div>
    )
...
export default class Game extends React.Component {
    ...
    initCellArray() {
        let cell_array = new Array(this.row);
        cell_array.fill(0);
        cell_array = cell_array.map(() => new Array(this.col));
    ...
    handleTouch() {
        ...
        if (start_native_event instanceof MouseEvent && end_native_event instanceof MouseEvent) {
        ...
        } else if (start_native_event instanceof TouchEvent && end_native_event instanceof TouchEvent) {
    ...
    getMoveRightResult() {
        let cell_array = this.state.cell_array;
        return this.mergeLeftMoveResult(cell_array.map(row => row.toReversed())).map(row => row.toReversed());
    }
